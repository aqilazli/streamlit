...
def load_phishing_model():
    try:
        ...
        hf_model = st.secrets.get("hf_model", "USERNAME/MODEL_NAME")

        if hf_token == "YOUR_HF_TOKEN_HERE":
            st.warning("⚠️ Please update your Hugging Face token in .streamlit/secrets.toml")
            return None
...
def is_phishing_ai(text):
    """Use AI model for phishing detection"""
    if phishing_model is None:
        # Fallback to keyword detection if model not loaded
        keywords = ['won','free','click','prize','urgent','verify','account','bank','password','claim','offer','winner','cash','reward']
        return keywords.some(word => text.lower().includes(word))

    try:
        result = phishing_model(text)
        # Adjust based on your model's output labels
        return result[0]['label'].upper() == 'PHISHING'
    except:
        return False
...
const phishingKeywords = ['won','free','click','prize','urgent','verify','account','bank','password','claim','offer','winner','cash','reward'];

function isPhishing(text) {
  const lower = text.toLowerCase();
  return phishingKeywords.some(k => lower.includes(k));
}
